...
export default function AdminPermissoesCargo() {
    ...
    async function setEffect(permissionKey, next) {
        ...
        // optimistic update no state (sem reload)
        const applyLocal = (eff) => {
            setGrants((curr) => {
                ...
                if (idx === -1) {
                    return [
                        ...curr,
                        { subjectType, subjectValue, permissionKey, effect: eff },
                    ];
                }

                const copy = curr.slice();
                copy[idx] = { ...copy[idx], effect: eff };
                return copy;
            });
        };
        ...
        try {
            ...
            // sucesso -> não faz nada (já atualizou local)
        } catch (err) {
            // rollback
            setGrants(prev);
            push({
