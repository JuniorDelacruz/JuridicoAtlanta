...
function Cartorio() {
  ...
  const handleImageLocal = (file) => {
    ...
    setImagemFile(file);
    setFormDados({ ...formDados, imagemIdentidade: 'pendente' }); // mantém sua lógica de placeholder
  };
  ...
  const removerImagem = () => {
    ...
    // se quiser já marcar erro ao remover:
  ...
  const handleRegistroArma = async () => {
    ...
    try {
      ...
      await axios.post('https://apijuridico.starkstore.dev.br/api/cartorio/arma/registro', formData, {
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' },
      });

      // Você pode trocar isso por um banner "sucesso" no modal se quiser.
  ...
  // Cadastro (mantido como você tinha)
  const handleRegistrar = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://apijuridico.starkstore.dev.br/api/cartorio/cadastro', formData, {
        headers: {
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b flex justify-between items-center">
                <h3 className="text-2xl font-bold">
                  {showForm === 'cadastro' ? 'Novo Cadastro de Cidadão' : showForm === 'arma' ? 'Registro de Arma' : 'Recolhimentos'}
                </h3>
                ...
                {showForm === 'cadastro' ? (
                  <form className="space-y-4">
                    ...
                    <div className="mt-8 flex justify-end gap-4">
                      <button type="button" onClick={fecharFormulario} className="px-6 py-2 bg-gray-300 rounded-md hover:bg-gray-400">
                        Cancelar
                      </button>
                      <button
                        type="button"
                  ...
                  <form className="space-y-4">
                    {/* Erro geral */}
                    {armaErrors.geral && (
                      <div className="rounded-md bg-red-50 border border-red-200 p-3 text-sm text-red-700">{armaErrors.geral}</div>
                    )}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          // ao mudar cidadão, reset do porte
                          setPorteOk(false);
                          setPorteMsg('');
                          setPorteStatus('idle');
                        }}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full border rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                          armaErrors.porteNumero ? 'border-red-500' : porteStatus === 'ok' ? 'border-green-500' : 'border-gray-300'
                        }`}
                        placeholder="Ex: 123456789012345678"
                        onBlur={checarPorte}
                        required
                    ...
                    <div>
                      ...
                      {armaErrors.imagemIdentidade && <p className="mt-2 text-sm text-red-600">{armaErrors.imagemIdentidade}</p>}
                    </div>
