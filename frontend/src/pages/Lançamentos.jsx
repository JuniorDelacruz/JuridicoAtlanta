    ...
    return (
        <div className="flex gap-2">
            <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium ${tab === "pendentes" ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-300"
                    }`}
            >
                Pendentes
            </button>
            <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium ${tab === "pagos" ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-300"
                    }`}
            >
                Pagos
            </button>
...
function NovoLancamentoWizard({ user, tiposDisponiveis, onCancel, onCreated }) {
    ...
    // idle | checking | ok | notfound | linked | invalid | error
    ...
    const canNext1 = !!tipo;
    const canNext2 = reqStatus === "ok"; // ✅ só avança se validou OK
    ...
    async function validarRequerimento(numero) {
        ...
        try {
            ...
            if (status === 404) {
                ...
                setReqInfo(null);
                setReqMsg(msg || "Requerimento não encontrado.");
                return;
            }
            if (status === 409) {
                setReqStatus("linked");
            ...
            setReqStatus("error");
            setReqInfo(null);
            setReqMsg(msg || "Erro ao validar requerimento.");
        }
    }

    // ✅ Debounce: valida enquanto digita (somente quando estiver no Step 2)
    useEffect(() => {
        if (step !== 2) return;
        ...
        <div className="space-y-4">
            ...
            {step === 2 && (
                <div className="space-y-2">
                    ...
                    {/* ✅ feedback visual */}
                    {reqStatus === "checking" && (
                        <div className="text-sm text-gray-500">{reqMsg || "Verificando..."}</div>
                    )}
                    ...
                    <div className="text-xs text-gray-500">
                        Você só consegue avançar se o requerimento existir e não estiver vinculado.
                    </div>
                </div>
            )}
